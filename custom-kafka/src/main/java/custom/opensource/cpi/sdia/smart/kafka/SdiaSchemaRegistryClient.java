...
public final class SdiaSchemaRegistryClient {
        ...
        try {
            synchronized (lock) {
                ...
                try {
                    ...
                    // readBodyLimited() fully consumes and closes the response stream.
                    // Avoid explicit disconnect() here so the JDK HTTP keep-alive cache can
                    // reuse warm Schema Registry sockets across cache misses.
                }
            }
        } finally {
    ...
    private static String credentialFingerprint(final String apiKey, final String apiSecret) {
        ...
        try {
            ...
            final byte[] bytes = digest.digest(principal.getBytes(StandardCharsets.UTF_8));
