...
public class SdiaKafkaConsumer extends ScheduledPollConsumer {
    ...
    private static final Duration COMMIT_TIMEOUT = Duration.ofSeconds(10L);
    ...
    private String doInitialize() {
        ...
        if (isOnPremiseProxyMode()) {
            try {
                ...
                this.nonRetryableConfigurationFailure = true;
                String errorMsg = buildCloudConnectorTunnelError(bootstrapServers, rootCauseMessage(tunnelEx));
        ...
        try {
            ...
            this.nonRetryableConfigurationFailure = true;
            String errorMsg = buildUnexpectedStartupError(rootCauseMessage(e));
    ...
    protected int poll() throws Exception {
        ...
        for (ConsumerRecord<String, byte[]> record : records) {
            ...
            try {
                ...
                successOffsets.put(topicPartition(record), new OffsetAndMetadata(record.offset() + 1));
                delivered++;
    ...
    private void restoreOriginalCommittedOffsetsAfterSingleRecovery(final ConsumerRecord<String, byte[]> record,
        ...
        try {
            ...
            kafkaConsumer.commitSync(restoreSnapshot, COMMIT_TIMEOUT);
    ...
    private void applyAssignedStartOffsets(final List<TopicPartition> topicPartitions,
        ...
        try {
            ...
            throw new RuntimeCamelException("Assigned start offset positioning failed. reason=" + reason
                    + " | auto.offset.reset=" + reset
    ...
    private void commitSuccessOffsets(Map<TopicPartition, OffsetAndMetadata> successOffsets, int delivered) {
        if (successOffsets == null || successOffsets.isEmpty() || closed.get() || kafkaConsumer == null) return;

        final Map<TopicPartition, OffsetAndMetadata> commitSnapshot = successOffsets;
        ...
        try {
            ...
            kafkaConsumer.commitSync(commitSnapshot, COMMIT_TIMEOUT);
    ...
    private void commitAvroPoisonOffsetThenRecycle(ConsumerRecord<String, byte[]> record, String fatalMsg, Throwable conversionEx) {
        ...
        try {
            kafkaConsumer.commitSync(poisonCommit, COMMIT_TIMEOUT);
    ...
    private String buildCloudConnectorTunnelError(final String bootstrapServers, final String cause) {
        return buildCompactRuntimeError(
                "Cloud Connector TCP tunnel failed",
                "Event.Smart.Kafka.Config.CloudConnector.Tunnel.Failed",
                commonKafkaContext(bootstrapServers)
                        + "\nProxy      : " + safeValue(endpoint.getProxyType())
                        + "\nLocation   : " + displayOptionalLocationId(endpoint.getSapSapccLocationId()),
                "The adapter could not open the SOCKS5 tunnel through SAP Cloud Connector.",
                "1. Verify the Cloud Connector TCP virtual host and port mapping.\n"
    ...
    private String buildCompactRuntimeError(final String title,
        ...
        err.append("\n\n🧭 Cause:\n\n").append(safeLine(cause, 150));
        appendFixBlock(err, fix);
        err.append("\n\n🔎 Root cause:\n\n").append(safeLine(compactRootCause(rootCause), 180));
        return err.toString();
    }
