...
public class SdiaKafkaEndpoint extends DefaultPollingEndpoint {
    ...
    /**
     * Timeout used by KafkaConsumer.commitSync(..., timeout).
     * Exposed as adapter UI parameter because CPI Trial, Cloud Connector TCP relay,
     * large payloads and slow group coordinators may exceed Kafka's default 10s commit wait.
     */
    @UriParam
    private Long offsetCommitTimeoutS = 60L;
    ...
    /**
     * Adapter-level offset commit timeout, in seconds.
     *
     * This is intentionally separate from requestTimeoutS. request.timeout.ms controls Kafka
     * protocol requests; commitSync(timeout) controls how long the adapter waits for the
     * synchronous offset commit call itself before surfacing a visible CPI failure.
     */
    public Long getOffsetCommitTimeoutS() { return Long.valueOf(normalizeOffsetCommitTimeoutS(offsetCommitTimeoutS)); }
    public void setOffsetCommitTimeoutS(Long v) { this.offsetCommitTimeoutS = Long.valueOf(normalizeOffsetCommitTimeoutS(v)); }
    public void setOffsetCommitTimeoutS(String v) { this.offsetCommitTimeoutS = Long.valueOf(normalizeOffsetCommitTimeoutS(parseLong(v))); }
    public Long getOffsetCommitTimeoutMs() { return Long.valueOf(normalizeOffsetCommitTimeoutS(offsetCommitTimeoutS) * 1000L); }
    ...
    private static long normalizeOffsetCommitTimeoutS(final Long value) {
        long seconds = value == null ? 60L : value.longValue();
        if (seconds < 5L) return 5L;
        if (seconds > 300L) return 300L;
        return seconds;
    }
